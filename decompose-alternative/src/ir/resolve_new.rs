            ...
            ResolvedCoreInstance::FromExports(exports) => {
                ...
                match expected_kind {
                    CoreExportKind::Func => {
                        ...
                        match resolved {
                            ResolvedCoreFunc::FromModule { module_idx, func_idx } => {
                                (module_idx, func_idx)
                            }
...
impl Resolve for ComponentInstanceNode {
    ...
    fn resolve(component: &Component, idx: u32) -> Self::Root {
        match component.instances.get(idx) {
            Some(ComponentInstanceNode::Instantiated { component_idx, args }) => {
        ...
        match alias {
            AliasInfo::CoreInstanceExport { instance_idx, name } => {
                let (module_idx, func_idx) =
                    component.resolve_core_instance_export(*instance_idx, name, CoreExportKind::Func);
                ResolvedCoreFunc::FromModule { module_idx, func_idx }
            }
            AliasInfo::Outer { count, index } => {
                let parent_ref = component.get_parent_component(*count);
...
impl CoreMemoryNode {
    fn follow_alias(component: &Component, alias: &AliasInfo) -> ResolvedCoreMemory {
        match alias {
            AliasInfo::CoreInstanceExport { instance_idx, name } => {
                let (module_idx, memory_idx) =
                    component.resolve_core_instance_export(*instance_idx, name, CoreExportKind::Memory);
                ResolvedCoreMemory { module_idx, memory_idx }
            }
            AliasInfo::Outer { count, index } => {
                let parent_ref = component.get_parent_component(*count);
                let parent = parent_ref.borrow();
...
impl CoreTableNode {
    fn follow_alias(component: &Component, alias: &AliasInfo) -> ResolvedCoreTable {
        match alias {
            AliasInfo::CoreInstanceExport { instance_idx, name } => {
                let (module_idx, table_idx) =
                    component.resolve_core_instance_export(*instance_idx, name, CoreExportKind::Table);
                ResolvedCoreTable { module_idx, table_idx }
            }
            AliasInfo::Outer { count, index } => {
                let parent_ref = component.get_parent_component(*count);
                let parent = parent_ref.borrow();
...
impl CoreGlobalNode {
    fn follow_alias(component: &Component, alias: &AliasInfo) -> ResolvedCoreGlobal {
        match alias {
            AliasInfo::CoreInstanceExport { instance_idx, name } => {
                let (module_idx, global_idx) =
                    component.resolve_core_instance_export(*instance_idx, name, CoreExportKind::Global);
                ResolvedCoreGlobal { module_idx, global_idx }
            }
            AliasInfo::Outer { count, index } => {
                let parent_ref = component.get_parent_component(*count);
                let parent = parent_ref.borrow();
